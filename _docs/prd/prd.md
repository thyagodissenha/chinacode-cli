...
> **Estado do produto:** Este PRD descreve um projeto totalmente novo. Nenhuma funcionalidade, integração, comando, sandbox, fluxo de agente, documentação de produto ou distribuição pública deve ser considerada implementada até que exista evidência em código, testes e release correspondente.
...
O **ChinaCode CLI** será uma ferramenta de terminal de código aberto que oferecerá uma experiência completa de agente de codificação autônomo, construída especificamente para aproveitar o poder dos **modelos de linguagem chineses** (Qwen, DeepSeek, MiMo) em ambientes de desenvolvimento de software.

Projetado como a **alternativa open-source ao Claude Code**, o ChinaCode CLI deverá combinar a sofisticação de agentes autônomos modernos (com acesso a ferramentas, execução de comandos e edição de arquivos) com diferenciais únicos: transparência total de custos, suporte multi-modelo, execução local, e um sistema de configuração baseado em Markdown que permita personalização profunda sem vendor lock-in.
...
- **Soberania Tecnológica:** Nenhum vendor lock-in; deverá funcionar com qualquer provedor OpenAI-compatible ou localmente
...
- **Custo-Efetividade Radical:** Meta de ser até 95% mais barato que soluções equivalentes ocidentais
...
| Necessidade | Soluções Atuais | ChinaCode CLI (proposto) |
|---|---|---|
| Agente autônomo de qualidade | Claude Code (caro, fechado) | Meta: open-source, barato |
| Suporte a modelos chineses | Wrappers genéricos | Meta: otimização nativa |
| Transparência de custos | Orçamentos opacos | Meta: token-level tracking |
| Execução local/offline | Limitado ou inexistente | Meta: Ollama/vLLM nativo |
| Personalização profunda | Arquivos de config proprietários | Meta: Agent.md em Markdown |
| Extensibilidade via padrões | Plugins proprietários | Meta: MCP (padrão aberto) |
...
| Plataforma | Versão Mínima | Status Alvo |
|---|---|---|
| macOS | 12+ (Monterey) | Primary planejado |
| Linux (Ubuntu/Debian) | 20.04+ | Primary planejado |
| Linux (Fedora/RHEL) | 35+ | Primary planejado |
| Windows | 10+ via WSL2 | Primary planejado |
| Windows (nativo) | 10+ | Best-effort planejado |
| Node.js | 20.0.0+ | Requisito planejado |
| Docker | 20.10+ | Opcional planejado |
...
| Dimensão | Claude Code | ChinaCode CLI (meta de produto) | Vantagem esperada |
|---|---|---|---|
| **Preço (mensal)** | $100-$200 | Meta: $2-$20 (pay-as-you-go) | 10-50x mais barato |
| **Modelos disponíveis** | Só Claude | Meta: Qwen, DeepSeek, MiMo, locais | Multi-vendor |
| **Execução offline** | Não disponível | Meta: Ollama/vLLM | Soberania |
| **Transparência de custo** | Budget opaco | Meta: token-level tracking | Previsibilidade |
| **Código-fonte** | Fechado | Meta: open-source (MIT) | Auditabilidade |
| **Personalização** | CLAUDE.md proprietário | Meta: Agent.md em Markdown | Portabilidade |
| **Extensões** | MCP + limitado | Meta: MCP full + plugins | Ecossistema |
| **Qualidade do agente** | Excelente | Meta inicial: bom, com melhoria contínua | Gap a fechar |
| **Latência** | Rápido | Meta dependente do modelo usado | Depende do modelo |
| **Suporte enterprise** | Dedicado | Meta futura: comunidade no início | Gap inicial |
...
5. **"Segurança não é feature premium"**
   - Sandbox Docker planejado para todos, não tier-gated
...
### 13.1 Fase 1: Foundation (Semanas 1-4) — Planejada
...
1. **Community (Gratuito):**
   - CLI open-source completo quando implementado e lançado
