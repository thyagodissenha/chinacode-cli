...
export class TUI {
  ...
  showHeader(): void {
    ...
    console.log(`  ChinaCode CLI v0.1.0 | modelo: ${model} | sandbox: ${sandbox}`)
    console.log(chalk.dim(line))
  }
  ...
  showToolCall(name: string, args: string): void {
    console.log(chalk.yellow('⚙ ') + chalk.bold(name) + ' ' + chalk.dim(args.slice(0, 80)))
  }
  ...
  updateStatus(model: string, cost: number, tokens: number): void {
    process.stdout.write(`\r${chalk.dim(`[${model}] ${tokens} tokens | $${cost.toFixed(4)}`)}`)
  }
  ...
  showSessionSummary(totalCost: number, durationMs: number, messages: number): void {
    const line = '─'.repeat(60)
    const durationSec = (durationMs / 1000).toFixed(1)
    console.log(chalk.dim(line))
    console.log(chalk.bold('  Resumo da sessão'))
    console.log(chalk.dim(line))
    console.log(`  ${'Custo total'.padEnd(20)} ${chalk.green('$' + totalCost.toFixed(4))}`)
