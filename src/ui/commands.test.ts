...
function makeMockStorage(sessionData?: Record<string, unknown>): SessionStorage {
  return {
...
describe('CommandParser.isCommand', () => {
  it('returns true for /help', () => {
    const parser = new CommandParser(baseConfig, makeMockTUI(), makeMockLoop(), makeMockStorage())
...
describe('CommandParser.execute', () => {
  ...
  beforeEach(() => {
    ...
    config = { ...baseConfig, models: { default: { ...baseConfig.models.default } } }
  })
  ...
  describe('/model command', () => {
    ...
    it('/model without argument shows warning', async () => {
      ...
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('/model'))
    })
  ...
  describe('/sandbox command', () => {
    ...
    it('/sandbox without valid arg shows warning', async () => {
      ...
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('/sandbox'))
    })
  ...
  describe('/resume command', () => {
    ...
    it('/resume with unknown id shows error', async () => {
      ...
      expect(tui.showError).toHaveBeenCalledWith(expect.stringContaining('não encontrada'))
    })

    it('/resume without id shows warning', async () => {
      const parser = new CommandParser(config, tui, loop, storage)
      await parser.execute('/resume')
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('/resume'))
    })
  })
  ...
  describe('/compact command', () => {
    it('/compact shows warning about Fase 2', async () => {
      const parser = new CommandParser(config, tui, loop, storage)
      await parser.execute('/compact')
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('Fase 2'))
    })
  })
  ...
  describe('unknown command', () => {
    it('unknown command shows warning and returns true', async () => {
      ...
      expect(result).toBe(true)
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('desconhecido'))
    })
  })
})
