...
export function getContextLimit(model: string): number {
	return MODEL_CONTEXT_LIMITS[model] ?? 32768;
}
...
export async function compactMessages(
	...
	const prompt = `Faça um resumo conciso do seguinte histórico de conversa, preservando as informações mais importantes, decisões tomadas e contexto relevante para continuar a tarefa:\n\n${serialized}`;

	const { content } = await fastClient.chat([
