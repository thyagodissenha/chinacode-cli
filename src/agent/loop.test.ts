...
function makeStreamingClient(chunks: Array<string | { tool_calls: Array<{ id: string; type: 'function'; function: { name: string; arguments: string } }> }>): InstanceType<typeof ModelClient> {
  const streamChat = vi.fn().mockImplementation(async function* () {
    for (const chunk of chunks) {
      yield chunk
    }
  })
  return { config: defaultConfig.models.default, streamChat } as unknown as InstanceType<typeof ModelClient>
}
...
describe('AgentLoop', () => {
  ...
  describe('basic text response', () => {
    ...
    it('adds system prompt when provided', async () => {
      ...
      const tui = makeMockTUI()
      const loop = new AgentLoop(defaultConfig, tui, [], 'You are a helpful assistant.')
      await loop.run('hello')

      expect(loop.state.messages[0]?.role).toBe('system')
      expect(loop.state.messages[0]?.content).toBe('You are a helpful assistant.')
    })
  ...
  describe('tool call execution', () => {
    it('executes a tool call and appends tool result to messages', async () => {
      ...
      const mockClient = { config: defaultConfig.models.default, streamChat } as unknown as InstanceType<typeof ModelClient>
      MockedModelClient.mockImplementation(() => mockClient)
      ...
      expect(tui.showToolCall).toHaveBeenCalledWith('echo_tool', expect.any(String))
      expect(tui.showToolResult).toHaveBeenCalledWith('echo_tool', expect.stringContaining('Echo: hello'), true)
    })
  ...
  describe('security guards', () => {
    it('blocks destructive bash commands', async () => {
      ...
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('destrutivo'))
    ...
    it('blocks access to secret file paths via read_file', async () => {
      ...
      const readTool: Tool = {
        ...
        execute: vi.fn().mockResolvedValue({ success: true, output: 'secret content' }),
      }
  ...
  describe('iteration limit', () => {
    it('stops after maxIterations and shows warning', async () => {
      ...
      const dummyTool: Tool = {
        ...
        parameters: { type: 'object', properties: {} },
        execute: vi.fn().mockResolvedValue({ success: true, output: 'keep going' }),
      }
      ...
      expect(tui.showWarning).toHaveBeenCalledWith(expect.stringContaining('iterações'))
  ...
  describe('state management', () => {
    it('setMessages replaces conversation history', async () => {
      ...
      const tui = makeMockTUI()
      const loop = new AgentLoop(defaultConfig, tui, [])
      const newMessages = [{ role: 'user' as const, content: 'restored message' }]
