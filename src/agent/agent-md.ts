...
const DEFAULT_IDENTITY = 'Você é ChinaCode CLI, um agente de codificação autônomo.'
...
function parseSections(raw: string): AgentMdSections {
  const result: AgentMdSections = { identity: '', rules: [], skills: [], subagents: [], raw }
...
export function buildSystemPromptFromAgentMd(
  ...
  parts.push(`- Seja direto, eficiente e cite sempre os arquivos que modificar.`)
