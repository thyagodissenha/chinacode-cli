...
import { formatErrorMessage, parseApiError } from "../models/errors.js";
import { calcRetryDelay, sleep } from "../models/retry.js";
...
import { compactMessages, needsCompaction } from "./compactor.js";
import {
...
export class AgentLoop {
	...
	async run(userInput: string): Promise<void> {
		...
		if (this.gitContext) {
			const existing = this.messages.findIndex(
				(m) => m.role === "system" && m.content?.startsWith("## Git Context"),
			);
			if (existing >= 0) {
		...
		while (this.iteration < this.config.maxIterations && !this.cancelled) {
			...
			if (needsCompaction(this.messages, modelConfig.model)) {
				this.tui.showWarning(
					"📦 Contexto próximo do limite — compactando automaticamente...",
				);
			...
			try {
				...
				const stream = client.streamChat(this.messages, this.tools, (usage) => {
				...
				if (assistantContent && toolCallsToExecute.length === 0) {
					...
					if (fallbackCalls.length > 0) {
						this.tui.showWarning(
							`⚠ Tool calls detectadas via fallback markdown parser (${fallbackCalls.length} call(s))`,
						);
				...
				if (this.iteration >= this.config.maxIterations) {
					this.tui.showError(
						`Limite de ${this.config.maxIterations} iterações atingido.`,
					);
					break;
				}
		...
		if (this.iteration >= this.config.maxIterations && !this.cancelled) {
			this.tui.showWarning(
				`Limite de iterações (${this.config.maxIterations}) atingido.`,
			);
		}
	}
	...
	private async handleWriteWithApproval(
		...
		const approval = await this.diffApproval.requestApproval(
			filePath,
			oldContent,
			displayContent,
		);
