...
import { classifyIntent, formatErrorMessage, parseApiError } from "./errors.js";
...
describe("parseApiError", () => {
	...
	it("marks 429 rate limit as retryable", () => {
		const err = {
			status: 429,
			error: { code: "limit_requests", message: "Rate limited" },
		};
...
describe("classifyIntent", () => {
	...
	it('classifies "analyse" as reasoning', () => {
		expect(classifyIntent("analyse the performance bottleneck")).toBe(
			"reasoning",
		);
	});
