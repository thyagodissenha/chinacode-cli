...
describe('SecretsGuard', () => {
  ...
  describe('sanitizeContent', () => {
    it('masks OpenAI-style API keys (sk-)', () => {
      const result = guard.sanitizeContent('key=sk-abcdefghijklmnopqrstuvwxyz123456')
    ...
    it('masks Bearer tokens', () => {
      const result = guard.sanitizeContent('Authorization: Bearer abcdef123456789012345678901234')
  ...
  describe('isDestructiveCommand', () => {
    ...
    it('detects git push --force', () => {
      expect(guard.isDestructiveCommand('git push --force origin main')).toBe(true)
    })
    ...
    it('detects dd if=', () => {
      expect(guard.isDestructiveCommand('dd if=/dev/zero of=/dev/sda')).toBe(true)
    })
    ...
    it('allows git commit', () => {
      expect(guard.isDestructiveCommand('git commit -m "feat: add feature"')).toBe(false)
    })
  ...
  describe('isOutsideWorkspace', () => {
    ...
    it('allows files inside workspace', () => {
      expect(wsGuard.isOutsideWorkspace('/workspace/project/src/index.ts')).toBe(false)
    })

    it('allows nested workspace paths', () => {
      expect(wsGuard.isOutsideWorkspace('/workspace/project/deep/nested/file.ts')).toBe(false)
    })
  })
})
