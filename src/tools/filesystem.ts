...
export const writeFileTool: Tool = {
  ...
  async execute(args: unknown): Promise<ToolResult> {
    try {
      ...
      return { success: true, output: `Arquivo escrito: ${filePath} (${bytes} bytes)` }
    } catch (err) {
      return { success: false, output: '', error: String(err) }
    }
  },
...
export const editFileTool: Tool = {
  ...
  async execute(args: unknown): Promise<ToolResult> {
    try {
      ...
      if (occurrences === 0) {
        return { success: false, output: '', error: `Nenhuma ocorrência encontrada de old_text em ${filePath}` }
      }
      if (occurrences > 1) {
        return { success: false, output: '', error: `${occurrences} ocorrências encontradas (ambíguo) em ${filePath}` }
      }
...
function globRecursive(dir: string, pattern: RegExp, results: string[], max: number): void {
  ...
  async execute(args: unknown): Promise<ToolResult> {
    try {
      ...
      try {
        ...
        return { success: false, output: '', error: `Padrão regex inválido: ${pattern}` }
      }
...
function listDir(dir: string, depth: number, maxDepth: number, lines: string[], indent: string): void {
  let entries: fs.Dirent[]
  try {
    entries = fs.readdirSync(dir, { withFileTypes: true })
  } catch {
...
export const listDirectoryTool: Tool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      recursive: { type: 'boolean', description: 'List recursively up to 3 levels', default: false },
    },
    required: ['path'],
  },
  async execute(args: unknown): Promise<ToolResult> {
    try {
      ...
      if (!fs.existsSync(dirPath)) {
        return { success: false, output: '', error: `Diretório não encontrado: ${dirPath}` }
      }
      const lines: string[] = []
