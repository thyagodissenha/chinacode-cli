...
describe('readFileTool', () => {
  it('reads an existing file with line numbers', async () => {
    ...
    const result = await readFileTool.execute({ path: join(tmpDir, 'test.txt') })
  ...
  it('returns error for missing file', async () => {
    const result = await readFileTool.execute({ path: join(tmpDir, 'missing.txt') })
  ...
  it('respects offset (skips lines)', async () => {
    ...
    const result = await readFileTool.execute({ path: join(tmpDir, 'big.txt'), offset: 2, limit: 3 })
    expect(result.success).toBe(true)
  ...
  it('reads empty file successfully', async () => {
    ...
    const result = await readFileTool.execute({ path: join(tmpDir, 'empty.txt') })
    expect(result.success).toBe(true)
  })

  it('reads file with unicode content', async () => {
    writeFileSync(join(tmpDir, 'unicode.txt'), '你好世界\nHello World')
    const result = await readFileTool.execute({ path: join(tmpDir, 'unicode.txt') })
    expect(result.success).toBe(true)
...
describe('writeFileTool', () => {
  ...
  it('creates intermediate directories automatically', async () => {
    ...
    const result = await writeFileTool.execute({ path, content: 'nested content' })
    expect(result.success).toBe(true)
  })
...
describe('editFileTool', () => {
  it('replaces unique text in a file', async () => {
    ...
    const result = await editFileTool.execute({ path, old_text: 'foo bar', new_text: 'baz qux' })
    expect(result.success).toBe(true)
    const readResult = await readFileTool.execute({ path })
  ...
  it('fails with "Nenhuma ocorrência" when old_text not found', async () => {
    ...
    const result = await editFileTool.execute({ path, old_text: 'missing text', new_text: 'replacement' })
    expect(result.success).toBe(false)
  ...
  it('fails when old_text is ambiguous (multiple occurrences)', async () => {
    ...
    const result = await editFileTool.execute({ path, old_text: 'foo', new_text: 'bar' })
    expect(result.success).toBe(false)
  ...
  it('preserves surrounding content after edit', async () => {
    ...
    await editFileTool.execute({ path, old_text: 'target', new_text: 'replaced' })
    const readResult = await readFileTool.execute({ path })
...
describe('globSearchTool', () => {
  it('returns success even with no matches', async () => {
    const result = await globSearchTool.execute({ pattern: '*.nonexistentextension9999' })
    expect(result.success).toBe(true)
...
describe('grepSearchTool', () => {
  it('finds pattern in files', async () => {
    writeFileSync(join(tmpDir, 'code.ts'), 'const SECRET_KEY = "abc"\nconst x = 1')
    const result = await grepSearchTool.execute({ pattern: 'SECRET_KEY', path: tmpDir })
    expect(result.success).toBe(true)
  ...
  it('includes file path and line number in output', async () => {
    ...
    const result = await grepSearchTool.execute({ pattern: 'match_here', path: tmpDir })
    expect(result.success).toBe(true)
  ...
  it('returns "Nenhuma correspondência" when not found', async () => {
    ...
    const result = await grepSearchTool.execute({ pattern: 'NOTFOUND_XYZ_99999', path: tmpDir })
    expect(result.success).toBe(true)
    expect(result.output).toContain('Nenhuma correspondência')
  })

  it('returns error for invalid regex', async () => {
    const result = await grepSearchTool.execute({ pattern: '[invalid(regex', path: tmpDir })
    expect(result.success).toBe(false)
  ...
  it('searches recursively in subdirectories', async () => {
    ...
    const result = await grepSearchTool.execute({ pattern: 'UNIQUE_MARKER_XYZ', path: tmpDir })
    expect(result.success).toBe(true)
    expect(result.output).toContain('UNIQUE_MARKER_XYZ')
  })

  it('uses current directory when path defaults', async () => {
    const result = await grepSearchTool.execute({ pattern: 'class CostTracker' })
    expect(result.success).toBe(true)
...
describe('listDirectoryTool', () => {
  ...
  it('returns error for missing directory', async () => {
    const result = await listDirectoryTool.execute({ path: join(tmpDir, 'nonexistent') })
    expect(result.success).toBe(false)
  ...
  it('lists recursively when requested', async () => {
    ...
    const result = await listDirectoryTool.execute({ path: tmpDir, recursive: true })
    expect(result.success).toBe(true)
